...
def run_metrics_lab():
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            TP = torch.sum((binary_preds == 1) & (y_val == 1)).item()
            FP = torch.sum((binary_preds == 1) & (y_val == 0)).item()
            FN = torch.sum((binary_preds == 0) & (y_val == 1)).item()

            # Avoid divide-by-zero errors safely
            precision = TP / (TP + FP) if (TP + FP) > 0 else 0.0
            recall = TP / (TP + FN) if (TP + FN) > 0 else 0.0

            # TODO: Calculate F1 Score (+1e-7 avoids divice by 0)
